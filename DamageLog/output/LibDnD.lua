...
function DnD.Resizer (wtResizer, wtMovable, fUseCfg)
	if( wtMovable == nil) then
		wtMovable = wtResizer:GetParent()
	end
	DnD.Init(wtMovable, wtResizer, fUseCfg, nil, nil, nil, nil, nil, nil, true)
end

function DnD.Init( wtMovable, wtReacting, fUseCfg, fLockedToParentArea, Padding, KbFlag, Cursor, oldParam1, oldParam2, resizer )
	...
	DnD.Widgets[ ID ].Resizer = resizer or false
	...
	if fUseCfg then
		...
		if Cfg then		
			if DnD.Widgets[ ID ].Resizer then
				InitialPlace = DnD.Widgets[ ID ].wtMovable:GetPlacementPlain()
				InitialPlace.sizeX = Cfg.sizeX or InitialPlace.sizeX
				InitialPlace.sizeY = Cfg.sizeY or InitialPlace.sizeY
	...
	DnD.Widgets[ ID ].Initial = { X = InitialPlace.posX, Y = InitialPlace.posY, HX = InitialPlace.highPosX, HY = InitialPlace.highPosY, SX = InitialPlace.sizeX, SY = InitialPlace.sizeY }
...
function DnD.IsEnabled( wtWidget, oldParam1 )
	if not DnD.Widgets then return end
	if wtWidget == DnD then wtWidget = oldParam1 end
	local ID = DnD.GetWidgetID( wtWidget )
	if ID then
		return DnD.Widgets[ ID ].Enabled
	end
end
...
function DnD.Register( wtWidget, fRegister )
	...
	local ID = DnD.GetWidgetID( wtWidget )
	if ID then
		if fRegister and DnD.Widgets[ ID ].Enabled then --and DnD.Widgets[ ID ].wtReacting:IsVisible() then
...
function DnD.OnPickAttempt( params )
	...
	if DnD.Widgets[ Picking ] and DnD.Widgets[ Picking ].Enabled and ( not DnD.Widgets[ Picking ].KbFlag or DnD.Widgets[ Picking ].KbFlag == KBF_NONE and params.kbFlags == KBF_NONE or common.GetBitAnd( params.kbFlags, DnD.Widgets[ Picking ].KbFlag ) ~= 0 ) then
		...
		-- if DnD.Widgets[ Picking ].Resizer then
			-- DnD.Place = DnD.Widgets[ Picking ].wtReacting:GetPlacementPlain()
			-- DnD.Reset = DnD.Widgets[ Picking ].wtReacting:GetPlacementPlain()
		-- end
...
function DnD.OnDragTo( params )
	...
	if DnD.Widgets[ DnD.Dragging ].Resizer then
		DnD.Place.sizeX = math.floor( DnD.Reset.sizeX + dx )
		DnD.Place.sizeY = math.floor( DnD.Reset.sizeY + dy )
...
function DnD.StopDragging( fSuccess )
	...
	if fSuccess then
		...
		if DnD.Widgets[ DnD.Dragging ].fUseCfg then
			SetConfig( DnD.Widgets[ DnD.Dragging ].CfgName, { posX = DnD.Place.posX, posY = DnD.Place.posY, highPosX = DnD.Place.highPosX, highPosY = DnD.Place.highPosY, sizeX = DnD.Place.sizeX, sizeY = DnD.Place.sizeY } )
		end
		DnD.Widgets[ DnD.Dragging ].Initial = { X = DnD.Place.posX, Y = DnD.Place.posY, HX = DnD.Place.highPosX, HY = DnD.Place.highPosY, SX = DnD.Place.sizeX, SY = DnD.Place.sizeY }
	else
