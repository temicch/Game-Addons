...
function ChatItem(itemId, fontSize)
	...
	local valuedText = common.CreateValuedText()
	local format = "<body alignx='left' fontname='AllodsWest' fontsize='"..(fontSize or GlobalFontSize).."' shadow='1' ><rs class='color'>[<r name='icon'/>]</rs> <tip_white><r name='text'/></tip_white></body>"
	common.SetTextValues(valuedText, {
	format = userMods.ToWString(format),
